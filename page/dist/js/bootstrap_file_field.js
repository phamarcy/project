...
(function( $ ) {
    ...
    $.fn.bootstrapFileField = function(options) {
        ...
        var settings = $.extend({
            // Presentation
            label: "เลือก",
            btnClass: 'btn-default btn-outline',
            preview: 'off',
        ...
        var fileSelectionErrorHandler = function(event, message, fileNameList) {
            fileNameList.empty()
                .append('<li class="text-danger"><b>ไฟล์ไม่ถูกต้อง!</b> '+ message +'</li>');
        };
        ...
        this.filter( ":file" ).each(function() {
            ...
            $(this).on('change', function(e) {
                ...
                if(maxNumFiles && this.files.length > maxNumFiles) {
                    $(this).trigger('fileSelectionError', [maxNumFiles +' ต้องเป็นไฟล์ Excel ที่มีนามสกุล .xls และ .xlsx เท่านั้น !', fileList]);
                    return abort(this);
                }
                ...
                for (var x = 0; x < this.files.length; x++) {
                    ...
                    if(fileTypes && fileTypes.indexOf(file.type) < 0) {
                        $(this).trigger('fileSelectionError', [file.name +' : ชนิดต้องเป็น .xls และ .xlsx เท่านั้น !', fileList]);
                        return abort(this);
                    }
